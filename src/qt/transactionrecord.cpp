...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.IsZerocoinSpend() && (fZSpendFromMe || wallet->zdcwTracker->HasMintTx(hash))) {
            //zDCW stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZDCW;
        ...
        BOOST_FOREACH (const CTxOut& txout, wtx.vout) {
            ...
            if (mine) {
                ...
                if (ExtractDestination(txout.scriptPubKey, address) && IsMine(*wallet, address)) {
                    // Received by Decentralway Address
...
bool IsZDCWType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZDCW:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZDCW || type == TransactionRecord::MNReward) {
