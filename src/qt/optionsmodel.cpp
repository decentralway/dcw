...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::DCW);
    ...
    if (!settings.contains("nAnonymizeDecentralwayAmount"))
        settings.setValue("nAnonymizeDecentralwayAmount", 1000);

    nAnonymizeDecentralwayAmount = settings.value("nAnonymizeDecentralwayAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeDecentralwayAmount"))
        SoftSetArg("-anonymizedecentralwayamount", settings.value("nAnonymizeDecentralwayAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in decentralway.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeDecentralwayAmount:
            return QVariant(nAnonymizeDecentralwayAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeDecentralwayAmount:
            nAnonymizeDecentralwayAmount = value.toInt();
            settings.setValue("nAnonymizeDecentralwayAmount", nAnonymizeDecentralwayAmount);
            emit anonymizeDecentralwayAmountChanged(nAnonymizeDecentralwayAmount);
            break;
        case CoinControlFeatures:
