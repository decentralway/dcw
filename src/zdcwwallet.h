...
#ifndef Decentralway_ZDCWWALLET_H
#define Decentralway_ZDCWWALLET_H
...
public:
    CzDCWWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZDCW(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZDCW(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //Decentralway_ZDCWWALLET_H
