...
public:
    ...
    bool WriteZDCWSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZDCWSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZDCWSeed_deprecated(uint256& seed);
    bool EraseZDCWSeed();
    bool EraseZDCWSeed_deprecated();

    bool WriteZDCWCount(const uint32_t& nCount);
    bool ReadZDCWCount(uint32_t& nCount);
