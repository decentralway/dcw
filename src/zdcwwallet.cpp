...
CzDCWWallet::CzDCWWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zdcw seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZDCWSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZDCWSeed_deprecated()) {
                    LogPrintf("%s: Updated zDCW seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zdcw seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zdcw wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzDCWWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZDCWCount(nCountLastUsed);
    else if (!walletdb.ReadZDCWCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzDCWWallet::Lock()
{
    seedMaster = 0;
}

void CzDCWWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzDCWWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZDCW(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzDCWWallet::LoadMintPoolFromDB()
{
...
void CzDCWWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzDCWWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzDCWWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zdcwTracker->HasPubcoinHash(pMint.first)) {
...
bool CzDCWWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZDCW(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zdcwTracker which also adds to database
    pwalletMain->zdcwTracker->Add(dMint, true);
...
void CzDCWWallet::SeedToZDCW(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzDCWWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzDCWWallet::GenerateDeterministicZDCW(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzDCWWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzDCWWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
