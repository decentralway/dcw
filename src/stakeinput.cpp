...
// The zDCW block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zDCW is a hash of the serial
...
{
    //Create an output returning the zDCW that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZDCWOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zDCW output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zDCW", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZDCWOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zDCW output", __func__);
...
{
    CzDCWTracker* zdcwTracker = pwallet->zdcwTracker.get();
    CMintMeta meta;
    if (!zdcwTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zdcwTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a DCW stake is the outpoint
