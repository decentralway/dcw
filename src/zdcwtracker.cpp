...
CzDCWTracker::CzDCWTracker(std::string strWalletFile)
{
...
CzDCWTracker::~CzDCWTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzDCWTracker::Init()
{
...
bool CzDCWTracker::Archive(CMintMeta& meta)
{
...
bool CzDCWTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzDCWTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzDCWTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzDCWTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzDCWTracker::GetSerialHashes()
{
...
CAmount CzDCWTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzDCWTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzDCWTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzDCWTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzDCWTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzDCWTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzDCWTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzDCWTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzDCWTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzDCWTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzDCWTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzDCWTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzDCWTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzDCWTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzDCWTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzDCWTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzDCWTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzdcw from DB\n", __func__, listDeterministicDB.size());
    }
